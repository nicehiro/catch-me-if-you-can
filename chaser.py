...
def chaser_dqn(env_name,
    ...
    for epi in range(episode_count):
        ...
        while True:
            ...
            chaser_poi, direction, reward, done, _ = env.step(action, chaser.name)
            ...
            total_steps += 1
            if done:
                print('Episode: %d\tsteps: %d\tLoss: %f' % (epi + 1, step + 1, runner_loss))
                break
            else:
...
if __name__ == '__main__':
    ...
    # Trainning model
    chaser_dqn(env, episode_count=20000, display=False, fps=100, need_reload=False,
               reload_path='./model/chaser-temp.pkl')
    # Observe result
